document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('load', () => {
    ...
    setTimeout(() => {
      ...
      // ✅ ここにロゴアニメーション追加！
      setTimeout(() => {
        const logo = document.querySelector('.logo-deco');
        if (logo) {
          logo.classList.add('fade-in');
        }

        // === ここからロゴアニメーション切り替え ===
        const logoImages = document.querySelectorAll('.logo-animation .logo-img');
        const logoSrcs = [
          'images/logo1.webp',
          'images/logo2.webp',
          'images/logo3.webp'
        ...
        if (logoImages.length > 0) {
          setInterval(() => {
            let previousIndex = -1; // 最初は何もなし

            logoImages.forEach(img => {
              let randomIndex;
              do {
                randomIndex = Math.floor(Math.random() * logoSrcs.length);
              } while (randomIndex === previousIndex); // 隣と同じを避ける

              previousIndex = randomIndex; // 今回選んだものを次回のチェック用に記憶

                img.src = logoSrcs[randomIndex]; // 切り替え
            });
          }, 1000); // 1.5秒ごとに切り替え
        }
        // === ここまで追加 ===

      }, 1000); // ← ローディング解除から1秒後にアニメーション開始
      ...
      // フェードイン関連要素すべて登録
      document.querySelectorAll('.fade-in, .fade-slide-up, .fade-in-left, .fade-in-right')
